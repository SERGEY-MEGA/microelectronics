...
void printWithoutReset() {
  Serial.println(F("----- CRC report -----"));
  Serial.print(F("Bytes received total: "));
  Serial.println(bytesReceived);
  Serial.print(F("CRC-8 = 0x"));
  printHexByte(currentCrc);
  Serial.println();
  Serial.println(F("Accumulator is preserved."));
  Serial.println(F("----------------------"));
}
...
void setup() {
  ...
  Serial.println(F("Lab 3 / CRC-8 UART server / accumulate mode"));
  ...
  Serial.println(F("Press the button on D2 to print CRC without resetting the accumulator."));
}
